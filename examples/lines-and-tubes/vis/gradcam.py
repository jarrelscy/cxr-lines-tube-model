...
def apply_colormap_on_image(x_orig, activation, colormap_name):
    ...
    heatmap_on_image = Image.alpha_composite(heatmap_on_image, heatmap.resize(org_im.size))
...
class CamExtractor:
    ...
    def forward_pass(self, x):
        ...
        feat_map = self.model.backbone(x)

        feat_map.register_hook(self.save_gradient)
        conv_output = feat_map

        logits = list()
        logit_maps = list()
        for index, num_class in enumerate(self.model.cfg.num_classes):
            if self.model.cfg.attention_map != "None":
                feat_map = self.model.attention_map(feat_map)

            classifier = getattr(self.model, "fc_" + str(index))
            # (N, num_class, H, W)
            logit_map = None
            if not (
                self.model.cfg.global_pool == "AVG_MAX"
                or self.model.cfg.global_pool == "AVG_MAX_LSE"
            ):
                logit_map = classifier(feat_map)
                logit_maps.append(logit_map)
            # (N, C, 1, 1)
            feat = self.model.global_pool(feat_map, logit_map)
...
class GradCam:
    ...
    def generate_cam(self, input_image, original_image, target_class):
        ...
        for index, num_class in enumerate(self.model.cfg.num_classes):
            fc = getattr(self.model, "fc_" + str(index))
            fc.zero_grad()
        ...
        heatmap, heatmap_on_image = apply_colormap_on_image(original_image, cam, "turbo")
